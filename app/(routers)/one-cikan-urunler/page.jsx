...
export default function OneCikanUrunlerPage() {
 ...
 const filteredProducts = useMemo(() => {
  ...
  return result;
 }, [allProducts, sortBy, priceRange, selectedCategories, selectedBrands, showInStock, showDiscounted]);
  ...
  <div className="min-h-screen bg-gray-50/50">
   ...
   <div className="relative h-48 overflow-hidden bg-linear-to-r from-amber-500 to-amber-700 md:h-64">
    ...
    <div className="container relative z-10 flex h-full flex-col justify-center px-4">
     <h1 className="text-3xl font-bold text-white md:text-4xl lg:text-5xl flex items-center gap-3">
      <Star className="size-8 sm:size-10 fill-white text-white" />
      Öne Çıkan Ürünler
     </h1>
     ...
     <div className="flex items-center gap-4">
      <Button
       ...
       className={cn(
        ...
        showFilters && "border-amber-500 bg-amber-50 text-amber-600"
       )}
      >
       <SlidersHorizontal className="size-4" />
       Filtreler
       {activeFiltersCount > 0 && (
        <span className="ml-1 flex size-5 items-center justify-center rounded-full bg-amber-500 text-xs text-white">
         {activeFiltersCount}
        </span>
       )}
      </Button>
      <span className="text-sm text-gray-500">
       <span className="font-semibold text-gray-800">{filteredProducts.length}</span> öne çıkan ürün
      </span>
     </div>
     ...
     <aside className={cn("w-72 shrink-0 transition-all duration-300", showFilters ? "block" : "hidden")}>
      <div className="sticky top-24 space-y-6 rounded-xl bg-white p-5 shadow-sm">
       <div className="flex items-center justify-between">
        ...
        {activeFiltersCount > 0 && (
         <button onClick={clearAllFilters} className="cursor-pointer text-sm text-amber-600 hover:text-amber-700">
          Tümünü Temizle
         </button>
        )}
       </div>
     ...
     <div className="flex-1">
      {activeFiltersCount > 0 && (
       <div className="mb-4 flex flex-wrap items-center gap-2">
        {selectedCategories.map((slug) => (
         <span key={slug} className="flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-sm text-amber-700">
        ...
        {selectedBrands.map((brand) => (
         <span key={brand} className="flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-sm text-amber-700">
          {brand}
      ...
      {filteredProducts.length === 0 ? (
       <div className="flex flex-col items-center justify-center rounded-xl bg-white py-16 shadow-sm">
        ...
        <p className="mb-6 text-gray-500">Filtre kriterlerinize uygun öne çıkan ürün bulunamadı.</p>
